...
#import "YNRefreshNormalHeader.h"
#import "YNRefreshConfig.h"

@interface YNRefreshNormalHeader ()
@property (nonatomic, strong) UILabel *titleLabel;
@property (nonatomic, strong) CAShapeLayer *arrowLayer;
@property (nonatomic, strong) CAShapeLayer *circleLayer;
@end

@implementation YNRefreshNormalHeader
...
- (void)prepare {
    ...
    self.mj_h = [YNRefreshConfig share].headerHeight;
...
- (void)placeSubviews {
    ...
    // 计算加载圈y坐标
    CGFloat qy = ([YNRefreshConfig share].headerHeight - [YNRefreshConfig share].loadingWidth) * 0.5;
    // 设置加载圈的位置
    ...
    // 计算label的x坐标
    CGFloat lx = qx + [YNRefreshConfig share].loadingWidth + [YNRefreshConfig share].space;
...
- (void)setState:(MJRefreshState)state {
    ...
    if (state == MJRefreshStateIdle) {
        self.titleLabel.text = [YNRefreshConfig share].headerIdleString;
        self.arrowLayer.hidden = NO;
        self.circleLayer.strokeEnd = 0.05;
        [self.circleLayer removeAllAnimations];
    }
    if (state == MJRefreshStatePulling) {
        self.titleLabel.text = [YNRefreshConfig share].headerPullingString;
    }
    if (state == MJRefreshStateRefreshing) {
        self.arrowLayer.hidden = YES;
        self.titleLabel.text = [YNRefreshConfig share].headerRefreshingString;
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change {
    ...
    if (self.state == MJRefreshStateIdle || self.state == MJRefreshStatePulling) {
        ...
        CGFloat offsetY = ABS(point.y)-self.scrollView.mj_inset.top;
        double progress = offsetY / 50;
        if (progress > 1.0) {
